    ...
    def get_queryset(self):
        ...
        employees = Employee.objects.annotate(
            active_task_count=Count(
                "task", filter=Q(task__status__in=["New Task", "In Progress"])
            ),
            earliest_deadline=Min(
                "task__deadline",
                filter=Q(task__status__in=["New Task",
                                           "In Progress"])
            ),
...
class TaskListAPIView(ListAPIView):
    ...
    @swagger_auto_schema(
        operation_description="Получение списка задач с "
        ...
        manual_parameters=[
            ...
            openapi.Parameter(
                "sub_tasks",
                openapi.IN_QUERY,
                description="Фильтрация задач по наличию подзадач",
    ...
    def get_queryset(self):
        ...
        if invalid_filters:
            raise ValidationError(
                (f"Фильтрация по полю(-ям) "
                 f"{', '.join(invalid_filters)} невозможна."
                 f"Доступные поля для фильтрации: "
...
class ImportantTasksListAPIView(ListAPIView):
    ...
    def get_queryset(self):
        ...
        parent_tasks_in_progress = Task.objects.filter(
            Q(parent_task__isnull=False) & Q(parent_task__status="In Progress")
        )
