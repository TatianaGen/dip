...
class TaskSummarySerializerTest(TestCase):
    def setUp(self):
        self.employee = Employee.objects.create(
            full_name="Jane Doe", position="Manager"
        )
    ...
    def test_task_summary_serialization(self):
        ...
        self.assertIn(
            "parent_task", data
        )  # Проверяем, что поле parent_task присутствует
...
class PotentialEmployeeSerializerTest(TestCase):
    def setUp(self):
        self.employee = Employee.objects.create(
            full_name="Alice Smith", position="Designer"
        )
...
class TaskWithPotentialEmployeesSerializerTest(TestCase):
    def setUp(self):
        ...
        self.employee2 = Employee.objects.create(
            full_name="Bob Johnson", position="Developer"
        )
        self.task = Task.objects.create(
            name="Test Task",
    ...
    def test_get_potential_employees(self):
        ...
        data = serializer.data
        self.assertIn(
            "potential_employees", data
        )  # Проверяем, что поле потенциальных сотрудников присутствует
...
class BusyEmployeeSerializerTest(TestCase):
    ...
    def test_no_tasks_for_employee(self):
        employee3 = Employee.objects.create(
            full_name="Charlie Brown", position="Tester"
        )
