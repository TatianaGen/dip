...
class EmployeeModelTest(TestCase):
    ...
    def test_employee_creation(self):
        ...
        self.assertEqual(
            self.employee.additional_info, "Experienced in Django and React."
        )
        self.assertIsNotNone(
            self.employee.created_at
        )  # Проверяем, что поле created_at не пустое
        self.assertIsNotNone(
            self.employee.updated_at
        )  # Проверяем, что поле updated_at не пустое
    ...
    def test_employee_additional_info_blank(self):
        ...
        employee_without_info = Employee.objects.create(
            full_name="Jane Smith", position="Project Manager"
        )
        self.assertEqual(
            employee_without_info.additional_info, ""
...
class TaskModelTest(TestCase):
    def setUp(self):
        # Создаем тестового сотрудника
        self.employee = Employee.objects.create(
            full_name="John Doe", position="Software Engineer"
        )
    ...
    def test_task_creation(self):
        ...
        self.assertIsNotNone(
            self.task.deadline
        )  # Проверяем, что поле deadline не пустое
    ...
    def test_task_with_parent_task(self):
        ...
        )  # Проверяем связь с родительской задачей
        self.assertIn(
            sub_task, parent_task.sub_tasks.all()
        )  # Проверяем, что дочерняя задача включена в родительскую

    def test_task_status_choices(self):
        # Проверяем допустимые значения для статуса
        for status in Task.STATUS_CHOICES:
            self.assertIn(
                status[0], [choice[0] for choice in Task.STATUS_CHOICES]
